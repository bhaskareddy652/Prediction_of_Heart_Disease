...
def takeInput():
    ...
    # Convert and scale numeric input features
    ...
    oldpeak1 = int(oldpeak.get()) / 6.2  # Assuming max value for oldpeak is 6.2
    
    inputValues.append(age1)
    # Convert string input features to numeric using appropriate encoding
    inputValues.append(int(sex.get()))  # Assuming 'Male' or 'Female' for sex
    inputValues.append(int(chestPain.get()))  # Assuming numeric value for chestPain
    inputValues.append(trestbps1)
    inputValues.append(chol1)
    inputValues.append(int(FBS.get()))  # Assuming numeric value for FBS
    inputValues.append(int(ECG.get()))  # Assuming numeric value for ECG
    inputValues.append(thalach1)
    inputValues.append(trestbps1)  # Reusing trestbps1 for consistency
    inputValues.append(oldpeak1)
    inputValues.append(int(slope.get()))  # Assuming numeric value for slope
    inputValues.append(int(ca.get()))  # Assuming numeric value for ca
    inputValues.append(int(thal.get()))  # Assuming numeric value for thal
    
    # Print input values for debugging
    print("Input Values:", inputValues)

    # Reshape inputValues into a numpy array for prediction (1 sample, 13 features)
    inputValues_array = np.array(inputValues).reshape(1, -1)

    # Perform prediction using the trained KNeighborsClassifier
    final_Result = knn_classifier.predict(inputValues_array)
    print("Prediction Result:", final_Result)

    # Create and configure the result display window
    ...
    if final_Result[0] == 1:
        ...
        label1 = tkinter.Label(substituteWindow, text="NO HEART DISEASE DETECTED", font=('Impact', -35), fg='green')
        label1.grid(row=0, column=1, columnspan=6)
        label2 = tkinter.Label(substituteWindow, text="Keep up with a healthy lifestyle!", font=('Impact', -20), fg='green')
        label2.grid(row=1, column=1, columnspan=6)
...
# heart = pd.get_dummies(heart, columns = ['sex', 'cp', 'fbs', 'restecg', 'exang', 'slope', 'ca', 'thal','oldpeak'])
